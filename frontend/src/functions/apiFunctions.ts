...
export const fetchApplicationData = async (skip = 0, limit = 5): Promise<any> => {
  try {
...
export const validateFormData = (formData: any, fieldsToValidate: string[]) => {
  ...
  for (let field of fieldsToValidate) {
    ...
    if (Array.isArray(value) && value.length === 0) {
      toast.error(`Please enter a valid value for ${formatFieldName(field)} field.`);
      return false;
    }
    ...
    if (typeof value === "object" && value !== null && !Array.isArray(value)) {
      ...
      for (let subField of subFields) {
        if (typeof value[subField] === "string" && !value[subField].trim()) {
          toast.error(`Please enter a valid value for ${formatFieldName(subField)} field in ${formatFieldName(field)}.`);
          return false;
        }
      }
    }
